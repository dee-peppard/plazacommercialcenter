...
- **210 hero is a concept rendering** (`units/210/210Rendering.jpeg`, the gallery walkway), chosen by the owner, and carries a small "Concept rendering" tag on the hero (the image's own baked-in "this is a rendering" text is at the very bottom edge and gets cropped in a wide hero). Any hero can show a tag via `hero_tag`. The 210 share preview (`og`) stays a real photo. The concept-renderings section has no disclaimer sentence (owner: "people know what a concept rendering is"), and the entry-facade sketch was removed from the 210 renderings list — the file `units/210/210Cota-EntranceRendering.jpg` stays because `design-community.html` uses it as a background.
